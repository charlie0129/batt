...
const (
	...
	forceDischargeTooltip          = `Cut power from the wall and run from the battery, either indefinitely or for a selected duration. After a temporary force discharge, batt automatically restores the power adapter.`
	forceDischargeScheduledTooltip = `A temporary force discharge is in progress. To cancel the schedule and restore wall power, choose "Stop Force Discharge".`
...
If you want batt to stop controlling charging, you can use the "Disable Charge Limit" command. To uninstall, you can use the "Uninstall Daemon" command in the Advanced menu.`
